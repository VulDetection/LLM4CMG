...
# results_df = pd.DataFrame(columns=['CVE ID', 'is_vul'])
# count = 0
# start_index = 0
# csv_filename = 'BigVul_gpt_newmsg.xlsx'
#
# # 遍历DataFrame，分析代码并将结果逐条写入Excel文件
# for index, row in df.iloc[start_index:].iterrows():
#     result = analyze_code(row['func_before'], msg[index])
#     new_row = [
#         row['CVE ID'],
#         result.get('is_vul', ''),
#     ]
...
# 读取 Excel 文件
df = pd.read_excel('BigVul_gpt_newmsg.xlsx')
