...
import os
# 读取CSV文件
df = pd.read_csv('PrimeVul_new_recom.csv')
...
results_df = pd.DataFrame(columns=['cve', 'Summary', 'Background', 'Impact', 'Fix'])
count = 924
start_index = 924
csv_filename = 'PrimeVul_new_com_msg.xlsx'

# 遍历DataFrame，分析代码并将结果逐条写入Excel文件
for index, row in df.iloc[start_index:].iterrows():
    result = analyze_code(row['func_before_recom'], row['commit_message'])
    new_row = [
        row['cve'],
        result.get('Summary', ''),
    ...
    if not os.path.exists(csv_filename):
        ...
        ws = wb.active
        # 写入表头
        ws.append(['cve', 'Summary', 'Background', 'Impact', 'Fix'])
        # 写入数据
        ws.append(new_row)
