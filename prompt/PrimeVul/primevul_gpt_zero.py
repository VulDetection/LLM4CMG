...
import os
# 读取CSV文件
df = pd.read_excel('../../data/PrimeVul/PrimeVul_test.xlsx')

file = ""
temp = 0.1
model=""

csv_filename = 'primevul_{}_zero.xlsx'.format(file)
...
for index, row in df.iloc[start_index:].iterrows():
    result = analyze_code(row['func_before_recom'], row['search_commit_msg'])
    new_row = [
        row['cve'],
        result.get('Summary', ''),
